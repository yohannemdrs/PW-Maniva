...
// Função para calcular o raio de uma área em hectares
function calculateRadius(areaHectares) {
    const areaSqMeters = areaHectares * 10000;
    return Math.sqrt(areaSqMeters / Math.PI);
}
/**
 * Middleware para verificar sobreposição de propriedades.
 * Aplica-se tanto na criação quanto na atualização de propriedades.
 */
async function checkOverlap(req, res, next) {
    const { areaHectares, localizacao } = req.body;
    const currentPropId = req.params.id; // Presente apenas em rotas PATCH
    if (!localizacao || !localizacao.coordinates || !areaHectares) {
        // Se os dados essenciais para a validação não estiverem presentes, 
        // deixa a validação de esquema cuidar disso ou assume que não há sobreposição a verificar.
        return next();
    }
